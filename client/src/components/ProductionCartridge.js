...
function ProductionCartridge() {
  ...
  const fetchData = async () => {
    try {
      ...
      const bufferPromises = Array.from(moNumbers).map(async (moNumber) => {
        try {
          const bufferResponse = await axios.get(`/api/buffer/cartridge`, {
            params: { moNumber }
          });
  ...
  const handleSubmitMoGroup = async (moNumber, sessionId) => {
    ...
    try {
      ...
      const response = await axios.put(`/api/production/cartridge/submit-mo-group`, {
        mo_number: moNumber,
        session_id: sessionId
      });
  ...
  const handleRevertMoGroup = async (moNumber) => {
    ...
    try {
      const response = await axios.put(`/api/production/cartridge/revert-mo-group/${moNumber}`, {
        userRole: userRole
      });
    ...
    try {
      const updatePromises = editFormData.inputIds.map((inputId, index) => {
        ...
        if (index === 0) {
          ...
          return axios.put(`/api/production/cartridge/${inputId}`, payload);
        }
        return axios.put(`/api/production/cartridge/${inputId}`, {
          pic: picVal,
          mo_number: editFormData.moNumber,
          sku_name: editFormData.skuName
        });
