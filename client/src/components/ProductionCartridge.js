...
function ProductionCartridge() {
  ...
  const getVendorIdByName = (name) => {
    if (!name) return '';
    const v = activeVendors.find((x) => x.name === name);
  ...
  const fetchData = async () => {
    try {
      ...
      const [bufferBatch, rejectBatch] = await Promise.all([
        axios.post('/api/buffer/cartridge/batch', { moNumbers: moList }).catch((err) => {
          console.error('Error fetching buffer batch:', err);
          return { data: {} };
        }),
        axios.post('/api/reject/cartridge/batch', { moNumbers: moList }).catch((err) => {
  ...
  const handleConfirmStart = () => {
    if (leaderName && shiftNumber) {
      ...
      localStorage.setItem('production_cartridge_session', JSON.stringify(sessionData));
    }
  };
  ...
  const handleValidateRow = (index, isEdit = false, showAlert = true) => {
    ...
    const vendorDigitCount = resolveDigitCountForVendorName(row.vendorName, vendorDigitMap);
  ...
  const handleRowVendorChange = (index, vendorId, isEdit = false) => {
    const name = getVendorNameById(vendorId);
    if (isEdit) {
      const newRows = [...editFormData.authenticityRows];
      newRows[index] = { ...newRows[index], vendorName: name };
      setEditFormData({ ...editFormData, authenticityRows: newRows });
      setEditAuthenticityValidationStatus((prev) => ({ ...prev, [index]: false }));
    } else {
      const newRows = [...formData.authenticityRows];
      newRows[index] = { ...newRows[index], vendorName: name };
      setFormData({ ...formData, authenticityRows: newRows });
      setAuthenticityValidationStatus((prev) => ({ ...prev, [index]: false }));
      setAuthenticityInvalidStatus((prev) => {
        const newStatus = { ...prev };
        delete newStatus[index];
        return newStatus;
  ...
  const handleEndManufacturing = async () => {
    if (window.confirm('Apakah Anda yakin ingin mengakhiri proses manufacturing?')) {
      try {
        ...
        localStorage.removeItem('production_cartridge_session');
                                    ...
                                    <div className="edit-form">
                                      ...
                                      <div className="authenticity-section">
                                        ...
                                        {editFormData.authenticityRows.map((row, rowIdx) => {
                                          ...
                                          return (
                                            <div key={rowIdx} className={`authenticity-row-input${activeVendors.length > 0 ? ' with-vendor' : ''}`}>
                                              {activeVendors.length > 0 && (
                                                <select
                                                  value={getVendorIdByName(row.vendorName)}
                                                  onChange={(e) => handleRowVendorChange(rowIdx, e.target.value, true)}
                                                  style={{ padding: '6px' }}
                                                >
          ...
          <div className="modal-content large-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="form-group">
              ...
              <input
                ...
                style={{ width: '100%', padding: '8px', fontSize: '16px', borderRadius: '4px', border: '1px solid #ccc' }}
              />
              <datalist id="pic-datalist-cartridge">
                {picList
                  .filter(pic => 
            ...
            <div className="authenticity-section">
              ...
              {formData.authenticityRows.map((row, index) => {
                ...
                return (
                  <div key={index} className={`authenticity-row-input${activeVendors.length > 0 ? ' with-vendor' : ''}`}>
                    {activeVendors.length > 0 && (
                      <select
                        value={getVendorIdByName(row.vendorName)}
                        onChange={(e) => handleRowVendorChange(index, e.target.value, false)}
                      >
                        <option value="">— Vendor —</option>
                        {activeVendors.map((v) => (
                          <option key={v.id} value={String(v.id)}>
        ...
        <div className="modal-overlay" onClick={() => {
          ...
          <div className="modal-content large-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="form-group">
              ...
              <input
                type="text"
                list="pic-datalist-buffer-cartridge"
                value={bufferData.pic}
                onChange={(e) => setBufferData({ ...bufferData, pic: e.target.value })}
                placeholder="Ketik untuk mencari atau pilih PIC..."
                style={{ width: '100%', padding: '8px', fontSize: '16px', borderRadius: '4px', border: '1px solid #ccc' }}
              />
              <datalist id="pic-datalist-buffer-cartridge">
                {picList
                  .filter(pic => 
        ...
        <div className="modal-overlay" onClick={() => {
          ...
          <div className="modal-content large-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="form-group">
              ...
              <input
                type="text"
                list="pic-datalist-reject-cartridge"
                value={rejectData.pic}
                onChange={(e) => setRejectData({ ...rejectData, pic: e.target.value })}
                placeholder="Ketik untuk mencari atau pilih PIC..."
                style={{ width: '100%', padding: '8px', fontSize: '16px', borderRadius: '4px', border: '1px solid #ccc' }}
              />
              <datalist id="pic-datalist-reject-cartridge">
                {picList
                  .filter(pic => 
...
export default ProductionCartridge;
